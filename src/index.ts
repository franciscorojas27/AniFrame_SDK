...
const animeScraper = await AnimeScraper.create()
const animeListScraper = new AnimeListScraper(animeScraper)
const animeDetailsScraper = new AnimeDetailsScraper(animeScraper)
const animeScheduleScraper = new AnimeScheduleScraper(animeScraper)
const animeStreamingScraper = new AnimeStreamingScraper(animeScraper)
...
io.on('connection', (socket) => {
  ...
  socket.on('getEpisodeList', async (data, callback) => {
    try {
      const result = await animeDetailsScraper.getEpisodeList(
        data.slug as string,
      )
      callback({ success: true, content: result })
    } catch (err) {
  ...
  socket.on('getAnimeDetails', async (data, callback) => {
    try {
      const result = await animeDetailsScraper.getAnimeDetails(
        data.slug as string,
      )
      callback({ success: true, content: result })
    } catch (err) {
  ...
  socket.onAny((eventName, ...args) => {
    try {
      ...
      if (!EVENTOS_VALIDOS.has(eventName)) {
        ...
        console.warn(
          `⚠️ Evento no reconocido en socket ${socket.id}: ${eventName}`,
        )
